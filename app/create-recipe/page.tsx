      ...
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        {step === 0 && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-border p-6">
              ...
              <p className="text-sm text-muted-foreground mb-1">Tell us about your recipe</p>
              ...
              <textarea
                ...
                rows={4}
                placeholder="Type your recipe description here... Eg: 'This is my grandmother's famous minestrone chip recipe. They're crispy on the outside, chewy on the inside, and perfect for family gatherings!'"
              />
            </div>
            ...
            <div className="bg-white rounded-2xl border border-border p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`p-4 rounded-xl border-2 text-left transition-colors ${!useAI ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}
                >
                  <span className="text-2xl mb-2 block">✏️</span>
                  <p className="text-sm font-semibold text-foreground">Create manually</p>
                  <p className="text-xs text-muted-foreground mt-1">Fill in all the recipe details yourself, step by step. Full control over every aspect of your recipe.</p>
                </button>
                <button
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Let AI automatically generate ingredients, instructions, categories, and all recipe details based on your description.</p>
                </button>
              </div>
              {useAI && (
                <button className="w-full mt-3 bg-primary text-primary-foreground py-3 rounded-xl text-sm font-semibold hover:bg-primary/90 transition-colors flex items-center justify-center gap-2">
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-2xl border border-border p-5">
              <div className="flex flex-col items-center py-4 mb-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">Review the generated content below. You can make changes in the next step.</p>
              </div>
              ...
              <div className="border border-border rounded-xl p-4 mb-4">
                ...
                <p className="text-xs text-muted-foreground mt-1.5">{aiRecipe.description}</p>
            ...
            <div className="bg-white rounded-2xl border border-border p-5">
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                {[
                  ...
                  { label: "Advanced", desc: "Experience cook recommended" },
                ].map((d) => (
                  <button
                    key={d.label}
                    onClick={() => setDifficulty(d.label)}
                    className={`p-3 rounded-xl border-2 text-left transition-colors ${difficulty === d.label ? "border-primary bg-primary/5" : "border-border hover:border-primary/30"}`}
                  >
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  <Tag size={15} className="text-foreground" />
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {popularTags.map((tag) => (
                    <button
                      ...
                      className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${selectedTags.includes(tag) ? "bg-primary text-white border-primary" : "bg-white text-muted-foreground border-border hover:border-primary/40"}`}
                    >
                      {tag}
                    </button>
                  ))}
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <Leaf size={15} className="text-foreground" />
                  <h3 className="text-sm font-semibold text-foreground">Dietary Information</h3>
                </div>
                <div className="flex flex-wrap gap-2 mb-3">
                  {dietaryTags.map((tag) => (
                    <button
                      key={tag}
                      onClick={() => toggleTag(tag, selectedDietary, setSelectedDietary)}
                      className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${selectedDietary.includes(tag) ? "bg-primary text-white border-primary" : "bg-white text-muted-foreground border-border hover:border-primary/40"}`}
                    >
                      {tag}
                    </button>
                  ))}
              ...
              <div className="bg-accent rounded-xl p-4 mt-4">
                ...
                <div className="flex items-center justify-center gap-4 mt-3 text-xs text-muted-foreground">
                  <span>Prep Time: {aiRecipe.prepTime}</span>
        ...
        {step === 3 && (
          <div className="bg-white rounded-2xl border border-border p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6">Your recipe &quot;{aiRecipe.title}&quot; is ready to share with the MyPlattr community.</p>
            <div className="flex gap-3">
              ...
              <Link href="/" className="flex-1 bg-primary text-primary-foreground text-sm font-bold py-3 rounded-xl hover:bg-primary/90 transition-colors flex items-center justify-center gap-2">
                Publish Recipe 🎉
              </Link>
            </div>
          </div>
